...
class TestHTTPHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        try:
            ...
            db.query("insert into t_comments(name,head,content,comment_time,parent_id) values('%s','%s','%s','%s','%s')"%(name,head,content,comment_time,parent_id))
            db.commit()
            db.close()
...
start_server(8004)  #启动服务，监听8000端口
